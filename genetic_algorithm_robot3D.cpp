// Markus Buchholz
// g++ genetic_algorithm_robot3D.cpp -o t -I/usr/include/python3.8 -lpython3.8
...
Pos posMutation(Pos chromA, Pos chromB)
{
    ...
    if (dominant == 0)
    {
        ...
        Xnew.z = chromA.z + generateRandomX() * (chromA.z - chromB.z);
    }
    else if (dominant == 1)
    {
        Xnew.x = chromB.x + generateRandomX() * (chromB.x - chromA.x);
        Xnew.y = chromB.y + generateRandomX() * (chromB.y - chromA.y);
        Xnew.z = chromB.z + generateRandomX() * (chromB.z - chromA.z);
    }
